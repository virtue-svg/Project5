...
from src.compare_clip_blip import BlipFusionClassifier
...
def main() -> None:
    ...
    blip = BlipModel.from_pretrained(args.model_name)
    ...
    collate_fn = partial(collate_blip_batch, processor, max_length=args.max_length)
    loader_kwargs = dict(
    ...
    model = BlipFusionClassifier(blip).to(device)
    ...
    history = []
    run_name = f"blip_{args.model_name.replace('/', '_')}"
        ...
        json.dumps(
            {
                ...
                "params_million": round((total_params or 0) / 1e6, 3),
                "trainable_params_million": round((trainable_params or 0) / 1e6, 3),
            },
            ensure_ascii=False,
            indent=2,
        ),
