...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare lightweight head variants with fixed training settings."
    )
    ...
    parser.add_argument("--replace-emoji", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--collapse-repeats", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--remove-stopwords", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument(
        "--variants",
        type=str,
        default="base,ln,mlp,gated",
        help="Comma-separated head variants to compare.",
    )
...
def main() -> None:
    ...
    out_root = args.output_dir or (root / "outputs" / "optimize" / "head_variant_compare")
    ...
    if args.image_aug:
        common.append("--image-aug")
    if not args.replace_emoji:
        common.append("--no-replace-emoji")
    if not args.collapse_repeats:
        common.append("--no-collapse-repeats")
    if args.remove_stopwords:
        common.append("--remove-stopwords")

    results = []
    variants = [v.strip() for v in args.variants.split(",") if v.strip()]
    for variant in variants:
        out_dir = out_root / f"head_{variant}"
        out_dir.mkdir(parents=True, exist_ok=True)
        cmd = common + ["--head-variant", variant, "--output-dir", str(out_dir)]
        results.append(_run_case(variant, cmd, out_dir))

    summary_path = out_root / "head_variant_compare.csv"
    ...
    print(f"Saved head variant comparison to: {summary_path}")
